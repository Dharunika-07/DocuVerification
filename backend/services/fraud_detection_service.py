...
# #         if confidence < 50:
# #             risk_score += 40
# #             fraud_indicators.append('Low AI confidence')
# #         elif confidence < 70:
# #             risk_score += 20
# #             fraud_indicators.append('Medium AI confidence')
        
# #         if not gemini_response.get('is_authentic', True):
# #             risk_score += 50
# #             fraud_indicators.append('Document flagged as inauthentic')
        
# #         fraud_indicators.extend(gemini_response.get('fraud_indicators', []))
        
# #         suspicious_patterns = [
# #             r'\b(fake|forged|counterfeit|duplicate)\b',
# #             r'\b(photoshop|edited|modified)\b',
# #             r'\b(copy|scan|reproduction)\b'
# #         ]
        
# #         for pattern in suspicious_patterns:
# #             if re.search(pattern, extracted_text, re.IGNORECASE):
# #                 risk_score += 10
# #                 fraud_indicators.append(f'Suspicious pattern detected')
        
# #         if len(extracted_text) < 50:
# #             risk_score += 15
# #             fraud_indicators.append('Insufficient text extracted')
...
# #         if risk_level == 'low' and risk_score < 20:
# #             return 'approved'
# #         elif risk_level == 'high' and risk_score > 80:
# #             return 'rejected'
# #         else:
...
# #After testing with fake passport
# import re
# from datetime import datetime
...
#         polish_patterns = [
#             r'\b(poland|polska|polish|rzeczpospolita)\b',
#             r'\bPOL\b',
#             r'\b[A-Z]{2}\d{6,8}\b.*POL',
#         ]
        
#         for pattern in polish_patterns:
#             if re.search(pattern, extracted_text, re.IGNORECASE):
#                 risk_score += 30
#                 fraud_indicators.append('Polish/European document detected')
#                 break
        
#         mrz_pattern = r'P<[A-Z]{3}[A-Z<]+<<[A-Z<]+<+'
#         if re.search(mrz_pattern, extracted_text):
#             if 'IND' not in extracted_text and 'INDIA' not in extracted_text.upper():
#                 risk_score += 40
#                 fraud_indicators.append('International passport MRZ detected - non-Indian')
...
#             return 'rejected'
#         elif risk_level == 'low' and risk_score < 25:
#             return 'approved'
#         else:
#             return 'manual_review'
